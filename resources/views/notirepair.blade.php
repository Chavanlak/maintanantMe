<form action="/statuspost" method="POST">
    ...
    <div class="mb-3">
        ...
        {{-- <input  value="{{$item->date ?\Carbon\Carbon::parse($item->date)->format('d/m/Y')}}" type="date" name="date" class="form-control" required> --}}
        <input 
    value="{{ $item->date ? \Carbon\Carbon::parse($item->date)->format('Y-m-d') : '' }}" 
    type="date" 
    name="date" 
    class="form-control" 
    required
